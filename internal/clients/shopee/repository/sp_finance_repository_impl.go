...
func NewShopeeFinanceRepository(client *shopee.ShopeeClient) FinanceRepository {
	return &ShopeeFinanceRepositoryImpl{client: client}
}

func (f *ShopeeFinanceRepositoryImpl) GetShopeeLiveSalesReport(financeReq params.ShopeeLiveFinanceRequest, cookie string) (*params.ShopeeLiveFinanceResponse, error) {
	...
	req, err := f.client.NewRequest("GET", endpoint, query, nil, cookie)
	if err != nil {
	...
	if err := f.client.DoRequest(req, &result); err != nil {
...
func (f *ShopeeFinanceRepositoryImpl) GetShopeeLiveSalesReportRange(financeReq params.ShopeeLiveFinanceRequest, cookie string) ([]params.ShopeeLiveReportItem, error) {
	...
	if daysLeft <= 7 {
		...
		resp, err := f.GetShopeeLiveSalesReport(req, cookie)
		if err != nil {
			return nil, err
		}
...
func (f *ShopeeFinanceRepositoryImpl) GetShopeeLiveRealTime(cookie string) ([]params.ShopeeLiveReportItem, error) {
